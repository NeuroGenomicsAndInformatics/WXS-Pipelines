...
# The first alignment submission tries to run on a GPU, but settles for a CPU if one isn't available
# The pipeline can be changed to only using GPU or CPU by modifying the -R submission flag
# The -R flag structure is {GPU OPTIONS} || {CPU OPTIONS}@10
# Removing the first or second half of the submission can force it to use GPU or CPU
...
bsub -g ${JOB_GROUP_ALIGN} \
  ...
  -R '{ select[gpuhost && mem>220GB] rusage[ngpus_physical=1:gmem=32GB, mem=220GB/job] span[hosts=1] } || { select[!gpuhost] rusage[mem=180GB/job] }@10' \
...
# If the first alignment job fails, usually due to a failure with a GPU, then this one will run
LSF_DOCKER_VOLUMES="/storage1/fs1/${STORAGE_USER}/Active:/storage1/fs1/${STORAGE_USER}/Active \
/scratch1/fs1/${SCRATCH_USER}:/scratch1/fs1/${SCRATCH_USER} \
/scratch1/fs1/ris/application/parabricks-license:/opt/parabricks \
${REF_DIR}:/ref \
...
## 2. BQSR
# This job runs when either alignment job succeeds.
...
# This job utilizes GPUs to run haplotypecaller from NVIDIA's Clara Parabricks
LSF_DOCKER_VOLUMES="/storage1/fs1/${STORAGE_USER}/Active:/storage1/fs1/${STORAGE_USER}/Active \
/scratch1/fs1/${SCRATCH_USER}:/scratch1/fs1/${SCRATCH_USER} \
